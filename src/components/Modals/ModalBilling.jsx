...
import { createFacture, getAllAssurances } from '../../services/FacturationService';
...
function ModalBilling({open,onClose,close,auth,appointment}) {
 ...
 const qc = new useQueryClient()
  ...
  const keyfacture = ["loadFactueMedecin",auth?.id];
  ...
  const {mutate} = useMutation((data) => createFacture(data),{
    onSuccess(_) {
  ...
  const detacheConsultation = (consultation) => {
     ...
     setTotal(Array.from(consultations).reduce((acc,cur) => acc + cur.price,0))
  }
  ...
  const saveFacture = () => {
    ...
    let data = {
      patientId:appointment?.patient?.user?._id,
      doctorId: auth?._id,
    ...
    return (
        ...
        onClose={onClose}
      >
        <div className="rounded-lg min-w-2/3 max-h-screen overflow-y-scroll bg-gray-50 px-5 py-8" style={style}>
    ...
    <div className="flex justify-between items-center mt-2 mb-10">
        <div>
            <div className="text-primary font-bold text-lg">{appointment?.patient?.user?.name}</div>
            <p className="text-primary text-sm">(+221) {appointment?.patient?.user?.phoneNumber}</p>
        </div>
        <div>
