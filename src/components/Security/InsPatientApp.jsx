...
function InsPatientApp({close}) {
 ...
 const {setPatientProp} = useAppointmentStore(state => ({setPatientProp: state.setPatientProp}),shallow)
   ...
   console.log(response);
    ...
    const {isLoading: loading, mutate: google} = useMutation((data) =>  registerPatientByGoogle(data), {
      onSuccess(data) { 
        if(signIn({token: data?.token,
          expiresIn: 60,
          tokenType: "Bearer",
...
>
<PushSpinner size={30} color="#686769" loading={isLoading || loading} />
</Backdrop>
        <div className="py-3">
                    ...
                    <InputText {...field} autoFocus className={classNames({ 'p-invalid': fieldState.invalid })} placeholder="Nom complet*" />
                      ...
                      {getFormErrorMessage('name')}
                        ...
                        <Controller control={control} name="accept" rules={{required:'Avez-vous accepter les conditions ?'}} render={({field,fieldState}) => (
                         ...
                         <label className={classNames({ 'p-error': errors.accept })}> J'accepte les termes et conditions*</label>
                        </div> 
