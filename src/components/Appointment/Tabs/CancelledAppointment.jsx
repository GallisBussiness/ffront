...
function CancelledAppointment({auth}) {
  const [layout, setLayout] = useState('grid');
   
  const { reject } = useTabStore( state => ({reject: state.reject}),shallow)

    const key  = ["loadCancelledAppointementsByDoctor"];

  const {data, isLoading} = useQuery(key,() => appointmentFindAllByDoctor(auth?._id,'CANCELLED'),{
    onSuccess:(_) =>  {
      reject(_.length);
    },
    staleTime: 50_000,
