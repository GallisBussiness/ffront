...
const VideoCall = ({auth}) => {
  ...
  const callUser = (id) => {
    ...
    peer.on('signal', (data) => {
      socket.emit('calluser', { userToCall: id, signalData: data, from: me, name: (auth?.name) });
    });
  ...
  <div className="xl:w-96">
    <input
      ...
      "
      placeholder="COLLER ICI L'ID A APPELER"
    />
  </div>
            </div> 
            {callAccepted && !callEnded ? (
              ...
              <div>
                ...
                <div className="flex space-x-2 justify-center">
              <div>
                <button type="button" onClick={() => callUser(idToCall)} className="px-4 py-1.5 bg-primary text-white font-semibold text-md leading-normal uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out flex items-center justify-between">
                 {calling ? <ImpulseSpinner size={40} /> : <>Appeler <MdCall className="mx-1 h-6 w-6" /> </>}
                </button>
              </div>
            </div>
              )}
